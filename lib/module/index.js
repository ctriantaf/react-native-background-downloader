...
import RNBackgroundDownloader, { Constants } from "./NativeRNBackgroundDownloader.js";
...
export function setConfig({
  ...
  if (progressInterval != null) if (typeof progressInterval === 'number' && progressInterval >= MIN_PROGRESS_INTERVAL) config.progressInterval = progressInterval;else console.warn(`[RNBackgroundDownloader] progressInterval must be a number >= ${MIN_PROGRESS_INTERVAL}. You passed ${progressInterval}`);
  if (progressMinBytes != null) if (typeof progressMinBytes === 'number' && progressMinBytes >= 0) config.progressMinBytes = progressMinBytes;else console.warn(`[RNBackgroundDownloader] progressMinBytes must be a number >= 0. You passed ${progressMinBytes}`);
  ...
  try {
    ...
    return foundTasks.map(taskInfo => {
      ...
      if (taskInfo.savedTaskState === Constants.TaskRunning) {
        ...
        if (taskInfo.bytesDownloaded === taskInfo.bytesTotal) task.state = 'DONE';else
...
export async function ensureDownloadsAreRunning() {
  ...
  for (const task of tasks) if (task.state === 'DOWNLOADING') {
    task.pause();
    task.resume();
  }
}
export function completeHandler(jobId) {
  ...
  if (!RNBackgroundDownloader) {
    console.warn('[RNBackgroundDownloader] Module not available for completeHandler');
    return;
  }
...
export function setApproval(isApproved) {
  return RNBackgroundDownloader.setApproval(isApproved);
}
export function download(options) {
  log('[RNBackgroundDownloader] download', options);
  if (!options.id || !options.url || !options.destination) throw new Error('[RNBackgroundDownloader] id, url and destination are required');
  options.headers = {
    ...config.headers,
    ...options.headers
  };
  if (!(options.metadata && typeof options.metadata === 'object')) options.metadata = {};
  ...
  if (typeof RNBackgroundDownloader.download !== 'function') {
    ...
    task.onError({
      error: 'Download method not available'
    });
    return task;
  }
  try {
